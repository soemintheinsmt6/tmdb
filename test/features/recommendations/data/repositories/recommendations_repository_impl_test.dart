...
void main() {
  ...
  test('ranks by cross-seed frequency, then rating as tie-breaker', () async {
    ...
    when(() => movieRemote.getMovieRecommendations(1)).thenAnswer(
      (_) async => buildPaginated(
        movies: [
          buildMovie(id: 100, voteAverage: 5),
          buildMovie(id: 200, voteAverage: 9),
        ],
      ),
    );
  ...
  test('mixes movie and TV recommendations from mixed seeds', () async {
    when(() => movieRemote.getMovieRecommendations(1)).thenAnswer(
      (_) async =>
          buildPaginated(movies: [buildMovie(id: 100, voteAverage: 9)]),
    );
    when(() => tvRemote.getTvRecommendations(5)).thenAnswer(
      (_) async =>
          buildPaginatedTv(shows: [buildTvShow(id: 400, voteAverage: 8)]),
    );
  ...
  test(
    'a movie id and a TV id that collide are kept as distinct titles',
    () async {
      when(
        () => movieRemote.getMovieRecommendations(1),
      ).thenAnswer((_) async => buildPaginated(movies: [buildMovie(id: 42)]));
      when(
        () => tvRemote.getTvRecommendations(2),
      ).thenAnswer((_) async => buildPaginatedTv(shows: [buildTvShow(id: 42)]));
  ...
  test(
    'is best-effort: one failed seed is skipped, the rest still return',
    () async {
      when(
        () => movieRemote.getMovieRecommendations(1),
      ).thenAnswer((_) async => buildPaginated(movies: [buildMovie(id: 100)]));
      when(
        () => movieRemote.getMovieRecommendations(2),
  ...
  test('returns a Failure only when every seed fetch fails', () async {
    when(
      () => movieRemote.getMovieRecommendations(any()),
    ).thenThrow(const NetworkException(message: 'offline'));
