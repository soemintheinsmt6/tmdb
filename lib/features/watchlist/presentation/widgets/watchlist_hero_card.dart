...
/// overlaid at the bottom. Mirrors `FavouriteHeroCard` but routes to the movie
/// or TV detail screen based on [WatchlistItem.mediaType].
class WatchlistHeroCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: IconButton(
        ...
        icon: const Icon(IconsaxPlusBold.save_2, color: AppColors.cyan),
        onPressed: () =>
            context.read<WatchlistCubit>().remove(item.mediaType, item.id),
      ),
    );
  }
}
